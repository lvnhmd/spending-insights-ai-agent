...
const external_apis_1 = require("../utils/external-apis");
describe('External API Integration', () => {
    describe('Feature Flags', () => {
        test('should load feature flags with correct defaults', () => {
            expect(external_apis_1.featureFlags.USE_CACHED_APIS).toBe(true);
            expect(external_apis_1.featureFlags.MODEL_TIER).toBe('haiku');
            expect(external_apis_1.featureFlags.ENABLE_PLAID).toBe(false);
            expect(external_apis_1.featureFlags.ENABLE_ALPHA_VANTAGE).toBe(true);
            expect(external_apis_1.featureFlags.DEMO_MODE).toBe(true);
        });
    });
    describe('API Status', () => {
        test('should return correct API availability status', () => {
            const status = (0, external_apis_1.getAPIStatus)();
    ...
    describe('Plaid Client', () => {
        test('should return cached transaction data', async () => {
            const apiClients = (0, external_apis_1.createAPIClients)();
