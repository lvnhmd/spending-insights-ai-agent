...
describe('PII Redaction', () => {
  describe('sanitizeTransactionData', () => {
    it('should redact credit card numbers', () => {
      ...
      testCases.forEach(({ input, expected }) => {
        const result = sanitizeTransactionData(input);
    ...
    it('should handle empty or null input', () => {
      ...
      testCases.forEach(input => {
        const result = sanitizeTransactionData(input as any);
  ...
  describe('containsPII', () => {
    it('should detect presence of PII', () => {
      ...
      testCases.forEach(({ input, expected }) => {
        expect(containsPII(input as any)).toBe(expected);
      });
    });
  });
  ...
  describe('getPIIRiskScore', () => {
    it('should return 0 for text with no PII', () => {
      expect(getPIIRiskScore('Regular coffee purchase')).toBe(0);
      expect(getPIIRiskScore('')).toBe(0);
      expect(getPIIRiskScore(null as any)).toBe(0);
    });

    it('should return higher scores for text with more PII', () => {
      const lowRisk = getPIIRiskScore('Card ending in 1234');
      const mediumRisk = getPIIRiskScore('Card 4532-1234-5678-9012');
      const highRisk = getPIIRiskScore('Card 4532-1234-5678-9012 account 987654321 john@example.com');
    ...
    it('should normalize scores based on text length', () => {
      ...
      const shortScore = getPIIRiskScore(shortText);
      const longScore = getPIIRiskScore(longText);
  ...
  describe('validateRedaction', () => {
    ...
    it('should return false if PII remains in redacted text', () => {
      ...
      expect(validateRedaction(original, redacted)).toBe(false);
    });

    it('should return true for text with no PII', () => {
      const text = 'Regular coffee purchase';
      expect(validateRedaction(text, text)).toBe(true);
    });
  });
  ...
  describe('batchSanitizeTransactions', () => {
    it('should sanitize multiple descriptions', () => {
      ...
      const results = batchSanitizeTransactions(descriptions);
    ...
    it('should handle empty array', () => {
      const results = batchSanitizeTransactions([]);
  ...
  describe('edge cases', () => {
    ...
    it('should handle very long strings with PII', () => {
      ...
      const result = sanitizeTransactionData(longDescription);
