...
interface SavingsOpportunity {
  type: 'subscription' | 'fee' | 'category_overspend' | 'duplicate' | 'optimization';
  description: string;
 ...
 */
async function analyzeSpendingPatterns(transactions: Transaction[], userId: string): Promise<SpendingPattern[]> {
 ...
 */
function calculateCategorySpending(transactions: Transaction[]): CategorySpending[] {
...
function mapOpportunityTypeToRecommendationType(type: SavingsOpportunity['type']): Recommendation['type'] {
  switch (type) {
    case 'subscription':
    case 'fee':
...
function generateDetailedDescription(opportunity: SavingsOpportunity): string {
...
function generateActionSteps(opportunity: SavingsOpportunity): string[] {
  switch (opportunity.type) {
    case 'subscription':
      return [
...
function calculateConfidence(opportunity: SavingsOpportunity): number {
...
function estimateImplementationTime(opportunity: SavingsOpportunity): string {
  switch (opportunity.type) {
    case 'subscription':
      return '5 minutes';
    case 'fee':
...
function categorizeImpact(potentialSavings: number): 'low' | 'medium' | 'high' {
  if (potentialSavings < 100) return 'low';
  if (potentialSavings < 500) return 'medium';
  return 'high';
}
